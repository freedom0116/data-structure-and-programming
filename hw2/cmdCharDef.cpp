...
// YOU NEED TO CUSTOMIZE THIS PART...
//
// Modify "cmcase BACK_SPACE_KEY:dCharDef.h" to work with the following code
//
...
getChar(istream& istr)
{
   ...
   switch (ch) {
      ...
      case ESC_KEY:
         ch = mygetc(istr);
         ch = mygetc(istr);
         switch (int(ch) + ARROW_KEY_FLAG)
         {
         case ARROW_UP_KEY:
         case ARROW_DOWN_KEY:
         case ARROW_LEFT_KEY:
         case ARROW_RIGHT_KEY:
            return returnCh(int(ch) + ARROW_KEY_FLAG);
         
         default:
            switch (int(ch) + MOD_KEY_FLAG)
            {
            case HOME_KEY:
            case END_KEY: 
               return returnCh(int(ch) + MOD_KEY_FLAG);
            }
            mygetc(istr);
            switch (int(ch) + MOD_KEY_FLAG)
            {
